...
import com.bcdevops.pickit.database.handling.crud.VoteCRUD;
...
public class DatabaseAccess {
    ...
    public ArrayList<Vote> retrievePickItVotes(int pickItID){
        ...
        DataAccess access = new DataAccess(voteCRUD.read());

        JSONObject json = access.getJson();
        ArrayList<Vote> votes = retrievePickItVotesFromJSON(json);

        return votes;
    }

    public void sendVote(Vote vote){
        VoteCRUD voteCRUD = new VoteCRUD(vote);
        DataAccess access = new DataAccess(voteCRUD.create());
            ...
            if(json.getInt("Successful") == 1){
                try{
                    ...
                    for(int a = 0; ;a++){
                        ...
                        PickIt pickIt = new PickIt(pickItID, userID, username, category, subjectHeader, secondsOfLife, choices);
    ...
    protected ArrayList<Vote> retrievePickItVotesFromJSON(JSONObject json){
        ...
        try {
            if(json.getInt("Successful") == 1){
                ...
                for(int a = 0; a < count; a++) {
                    JSONObject voteJSON = new JSONObject(json.getString("'" + a + "'"));

                    int pickItID = Integer.parseInt((String) voteJSON.get("PickItID"));
                    int userID = Integer.parseInt((String) voteJSON.get("UserID"));
                    int choiceID = Integer.parseInt((String) voteJSON.get("ChoiceID"));
                    String birthday = (String) voteJSON.get("Birthday");
                    String gender = (String) voteJSON.get("Gender");
                    String ethnicity = (String) voteJSON.get("Ethnicity");
                    String religion = (String) voteJSON.get("Religion");
                    String political = (String) voteJSON.get("Political Affiliation");

                    Vote vote = new Vote(pickItID, choiceID, userID);
                    vote.setDemographics(new Demographics(birthday, gender, ethnicity, religion, political));
                    votes.add(vote);
                }
            }
        } catch (JSONException e) {
